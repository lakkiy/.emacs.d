...
(with-eval-after-load 'claude-code-ide
  ...
  不要求 Claude 窗口可见,也不会切换焦点。"
    ...
    (sit-for 0.2)
    ;; 多实例之后 buffer 名带实例名,不能再用 --get-buffer-name 拼;
    ;; 走和 insert-at-mentioned 一样的 session 解析,保证发到同一个实例。
    (let* ((session (claude-code-ide--resolve-session 'auto "Claude instance: "))
